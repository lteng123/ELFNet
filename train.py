...
def test(model, path, dataset):
    ...
    for i in range(num1):
        ...
        res1, res2, res3, res4, res5, res6 = model(image) # forward
...
def train(train_loader, model, optimizer, epoch, test_path, model_name = 'Net'):
    model.train()
    global best
    ...
    for i, pack in enumerate(train_loader, start=1):
        for rate in size_rates:
            ...
            loss_P5 = dice_loss(P5, egs)
            loss_P6 = structure_loss(P6, gts)
            
            alpha, beta, gamma, zeta = 1., 1., 1., 1. 

            loss = alpha * loss_P1 + beta * loss_P2 + gamma * loss_P3 + zeta * loss_P4 + 4 * loss_P5 + loss_P6# current setting is for additive aggregation.
    ...
    torch.save(model.state_dict(), save_path + str(epoch) + 'Model.pth')
...
if __name__ == '__main__':
    ...
    train_loader = get_loader(image_root, gt_root, eg_root, batchsize=opt.batchsize, trainsize=opt.img_size,
